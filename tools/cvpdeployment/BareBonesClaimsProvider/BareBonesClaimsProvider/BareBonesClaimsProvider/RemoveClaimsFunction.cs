    ...
    public class RemoveClaimsFunction
    {
            ...
            ILogger log)
        {
            ...
            using (StreamReader reader = new StreamReader(req.Body))
            {
                ...
                await claimsProviderStore.RemoveClaimsAsync(claimsRequest.VehicleId, claimsRequest.UserId, claimsRequest.ServiceId);

                const string responseMessage = "Deleted claim";
                log.LogInformation("[{tag}] Deleted claim for: {requestBody}", "", requestBody);
